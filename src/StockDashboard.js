...
/* ✅ FIX: Always-correct qty & remaining for each stock entry */
const getEntryQty = (e) => safeNum(e?.quantity);

/** ✅ RAW remaining field (no fallback). Used to detect if backend provides it. */
const getEntryRemainingRaw = (e) => {
  const raw =
    e?.remainingQuantity ??
    e?.remainingQty ??
    e?.remainingquantity ?? // just in case
    e?.remaining ??
    e?.remainingStock ??
    e?.remaining_balance ??
    null;
...
const getEntryRemaining = (e) => {
  ...
  // If backend doesn't provide remaining, fallback = qty (for purchase entries)
  if (raw === null) {
    if (qty <= 0) return 0;
    return qty;
  }
  ...
  // Guard: remaining should not exceed qty for positive purchases
...
const getEffectiveStatus = (e) => {
  ...
  // ✅ User request: qty 0 => SOLD, and all sales => SOLD
...
// Same idea as Ledger.js getQty (supports arrays)
...
const isSaleEntry = (e) => {
  ...
  if (cat) return cat.includes("SALE"); // ✅ more tolerant
  ...
  // ✅ last resort heuristic: if it has product items qty and debit > 0, treat as sale
...
function getSalesStats(sales = [], productType = null) {
  ...
  const norm = sales.map((s) => {
    const qty =
      key ? getLedgerSaleQtyForProduct(s, productType) : getLedgerSaleQty(s);
...
/* -------------------------------------------------------- */
/* ✅ FIXED: Stock-by-product calculation (NO DOUBLE COUNT)  */
/* - Sold priority: negative stock -> remaining fields -> ledger fallback
/* - Remaining Value is now consistent with Remaining */
/* -------------------------------------------------------- */
function computeByProductFromEntries(entries = [], ledgerSoldMap = {}) {
  const groups = {};

  const entryTotalCost = (e) => {
    const qty = getEntryQty(e);
    ...
    return qty * rate + loading + unloading + transport + other;
  };
  ...
  for (const e of entries) {
    ...
    if (!groups[nKey]) {
      groups[nKey] = { product, purchases: [], negatives: [] };
    }
  ...
  for (const nKey of Object.keys(groups)) {
    ...
    // ✅ FIX: Remaining value must follow the SAME remaining logic
    let remainingValue = 0;

    if (totalPurchased > 0) {
      if (hasRemainingInfo && soldFromNegatives <= 0) {
        // Accurate per-entry proportional remaining value
...
function StockDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      // ✅ FIX: Auto mark SOLD in backend for any entry that is sold by qty/remaining
      ...
      if (toMarkSold.length > 0) {
        ...
        // update locally (so UI matches immediately)
        ...
        // refresh summary to match updated statuses
        try {
  ...
  const calculateTotalCost = () => {
    ...
    const rate = Number(form.purchaseRate) || 0;
    const loading = Number(form.loadingCharges) || 0;
    const unloading = Number(form.unloadingCharges) || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.productType || !form.purchaseDate || !form.quantity || !form.purchaseRate) {
      await Swal.fire({
        icon: "warning",
        title: "Missing fields",
        text: "Please fill product type, purchase date, quantity and purchase rate.",
      });
      return;
    }
  ...
  const saveEdit = async (id) => {
    ...
    try {
      await api.put(`/api/stock/${id}`, {
        ...
        purchaseRate: Number(editForm.purchaseRate) || 0,
        loadingCharges: Number(editForm.loadingCharges) || 0,
        unloadingCharges: Number(editForm.unloadingCharges) || 0,
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Error updating stock entry.",
      });
    }
  };
  ...
  const uniqueProducts = [...new Set(entries.map((e) => e.productType).filter(Boolean))].sort();
  const uniqueSuppliers = [...new Set(entries.map((e) => e.supplierName).filter(Boolean))].sort();
  ...
  // ✅ FIXED: local by product now uses ledger fallback too (when needed)
  ...
  const totalSoldQtyOverall = useMemo(() => {
    return ledgerBasedSoldQtyOverall > 0 ? ledgerBasedSoldQtyOverall : stockBasedSoldQtyOverall;
  ...
  const totalPurchasedValueOverall = useMemo(() => {
    return entries.reduce((sum, e) => {
      ...
      const totalCost = qty * rate + loading + unloading + transport + other;
      ...
      <div style={styles.container}>
          ...
          <div style={styles.titleWrap}>
            ...
            <div>
              ...
              <div style={styles.subTitle}>
                Purchases • Movement • Availability • Sales Mapping
              </div>
            </div>
          </div>
          ...
          <>
            <div style={styles.cardsRow}>
              <div style={styles.card}>
                ...
                <div style={styles.cardValue}>{summary.bookedQty.toLocaleString()}</div>
                <div style={styles.cardSubValue}>₨ {summary.bookedValue.toLocaleString()}</div>
              </div>

              <div style={styles.card}>
                <div style={styles.cardAccent} />
                <div style={styles.cardLabel}>ON WAY</div>
                <div style={styles.cardValue}>{summary.onWayQty.toLocaleString()}</div>
                <div style={styles.cardSubValue}>₨ {summary.onWayValue.toLocaleString()}</div>
              </div>

              <div style={styles.card}>
                <div style={styles.cardAccent} />
                <div style={styles.cardLabel}>UNLOADED</div>
                <div style={styles.cardValue}>{summary.unloadedQty.toLocaleString()}</div>
                <div style={styles.cardSubValue}>₨ {summary.unloadedValue.toLocaleString()}</div>
              </div>
              ...
              <div style={styles.card}>
                ...
                <div style={styles.cardSubValue}>
                  Preferred: ledger SALES qty (fallback: stock diff)
                </div>
              </div>
              ...
              <div style={styles.card}>
                ...
                <div style={styles.cardValue}>
                  {Number(totalPurchasedOverall || 0).toLocaleString()}
                </div>
                <div style={styles.cardSubValue}>
                  ₨ {Number(totalPurchasedValueOverall || 0).toLocaleString()}
                </div>
              </div>
            </div>
              ...
              <div style={styles.section}>
                <div style={styles.sectionHeader}>
                  ...
                  <div style={styles.sectionHint}>
                    Safe calculation (avoids double sold count)
                  </div>
                </div>
                      ...
                      {Object.entries(localByProduct).map(([product, data], idx) => {
                        return (
                          <tr key={product} style={idx % 2 ? styles.rowAlt : null}>
                            ...
                            <td style={styles.td}>
                              {Number(data.totalPurchased || 0).toLocaleString()}
                            </td>
                            <td style={styles.td}>
                              {Number(data.remaining || 0).toLocaleString()}
                            </td>
                            <td style={styles.td}>
                              {Number(data.sold || 0).toLocaleString()}
                            </td>
                            <td style={styles.td}>
                              ₨ {Number(data.purchaseValue || 0).toLocaleString()}
                            </td>
                            <td style={styles.td}>
                              ₨ {Number(data.remainingValue || 0).toLocaleString()}
                            </td>
                          </tr>
                        );
                      })}
        ...
        <div style={styles.section}>
          <div style={styles.sectionHeader}>
            ...
            <div style={styles.sectionHint}>Fill required fields marked with *</div>
          </div>
              ...
              <div style={styles.costGrid}>
                <div style={styles.costItem}>
                  ...
                  <div style={{ fontWeight: "800", fontSize: "14px" }}>
                    ₨ {costDetails.baseValue.toLocaleString()}
                  </div>
                </div>
                <div style={styles.costItem}>
                  <div style={styles.sectionHint}>Total Charges</div>
                  <div style={{ fontWeight: "800", fontSize: "14px" }}>
                    ₨ {costDetails.totalCharges.toLocaleString()}
                  </div>
                </div>
                <div style={styles.costItem}>
                  <div style={styles.sectionHint}>Total Cost</div>
                  <div style={{ fontWeight: "800", fontSize: "14px", color: "#1f3b7a" }}>
                    ₨ {costDetails.totalCost.toLocaleString()}
                  </div>
                </div>
                <div style={styles.costItem}>
                  <div style={styles.sectionHint}>Effective Rate/Unit</div>
                  <div style={{ fontWeight: "800", fontSize: "14px" }}>
                    ₨ {costDetails.effectiveRate.toFixed(2)}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={styles.section}>
          <div style={styles.sectionHeader}>
            <h3 style={styles.sectionTitle}>Filter Stock Entries</h3>
            <div style={styles.sectionHint}>Narrow results by status, product or supplier</div>
          </div>
        ...
        <div style={styles.section}>
          <div style={styles.sectionHeader}>
            <h3 style={styles.sectionTitle}>
              Stock Entries <span style={styles.sectionHint}>({entries.length})</span>
            </h3>
            <div style={styles.sectionHint}>Scroll horizontally for full details</div>
          </div>
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {entries.map((entry, idx) => {
                  ...
                  const statusStyle =
                    statusColors[effectiveStatus] || { bg: "#e9ecef", color: "#495057" };
                  ...
                  return (
                    <tr key={entry._id} style={idx % 2 ? styles.rowAlt : null}>
                      ...
                      <td style={styles.td}>
                        <strong style={{ color: effectiveStatus === "SOLD" ? "#842029" : "#0f5132" }}>
                          {Number(remaining || 0).toLocaleString()}
                        </strong>
                      </td>
                      <td style={styles.td}>₨ {Number(rate || 0).toLocaleString()}</td>
                      <td style={styles.td}>₨ {Number(totalCost || 0).toLocaleString()}</td>
                      <td style={styles.td}>
                ...
                {entries.length === 0 && !loading && (
                  <tr>
                    <td style={styles.td} colSpan="9">
                      No stock entries found. Add your first entry above.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div style={styles.modalBackdrop} onClick={closeDetails}>
              <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={styles.detailGrid}>
                  ...
                  <div>
                    <div style={styles.detailLabel}>Invoice No</div>
                    <div style={styles.detailValue}>{viewingEntry.supplierInvoiceNo || "-"}</div>
                  </div>

                  <div>
                    <div style={styles.detailLabel}>Vehicle Number</div>
                    <div style={styles.detailValue}>{viewingEntry.vehicleNumber || "-"}</div>
                  </div>

                  <div>
                    <div style={styles.detailLabel}>Transport Company</div>
                    <div style={styles.detailValue}>{viewingEntry.transportCompany || "-"}</div>
                  </div>

                  <div>
                    <div style={styles.detailLabel}>Warehouse</div>
                    <div style={styles.detailValue}>{viewingEntry.warehouseLocation || "-"}</div>
                  </div>
                  ...
                  <div style={{ gridColumn: "1 / -1" }}>
                    ...
                    <div style={{ fontSize: "22px", fontWeight: "800", color: "#1f3b7a" }}>
                      ₨{" "}
                      {Number(
                        qty * safeNum(viewingEntry.purchaseRate) +
                          safeNum(viewingEntry.loadingCharges) +
                          safeNum(viewingEntry.unloadingCharges) +
                          safeNum(viewingEntry.transportCharges) +
                          safeNum(viewingEntry.otherCharges)
                      ).toLocaleString()}
                    </div>
                  </div>
                </div>
                ...
                {viewingEntry.salesDetails &&
                  ...
                  viewingEntry.salesDetails.length > 0 && (
                    <>
                      ...
                      <div style={styles.sectionHeader}>
                        <h3 style={{ ...styles.sectionTitle, color: "#1f3b7a" }}>
                          Sold To (Client Ledger Details)
                        </h3>
                      ...
                      <div style={styles.sectionHint}>
                        Showing SALES entries from ledger for product type{" "}
                        <strong>{viewingEntry.productType}</strong>.
                      </div>
                        ...
                        return (
                          <div style={{ ...styles.cardsRow, marginTop: "10px" }}>
                            ...
                            <div style={styles.card}>
                              ...
                              <div style={styles.cardValue}>
                                ₨ {Number(stats.avgRate || 0).toFixed(2)}
                              </div>
                            </div>
                          </div>
                        );
                        ...
                        <table style={styles.table}>
                          <thead>
                            <tr>
                              ...
                              <th style={styles.th}>Cheque Date</th>
                            ...
                            {viewingEntry.salesDetails.map((sale, idx) => {
                              ...
                              return (
                                <tr key={idx} style={idx % 2 ? styles.rowAlt : null}>
                                  ...
                                  <td style={{ ...styles.td, textAlign: "right" }}>
                                    ₨ {(s.loading || 0).toLocaleString()}
                                  </td>
                                  <td style={{ ...styles.td, textAlign: "right" }}>
                                    <strong>₨ {(s.debit || 0).toLocaleString()}</strong>
                                  </td>
                                  <td style={{ ...styles.td, textAlign: "right" }}>
                                    ₨ {(s.credit || 0).toLocaleString()}
                                  </td>
                                  <td style={styles.td}>
                                  ...
                                  <td style={styles.td}>{s.bankName || "-"}</td>
                                  <td style={styles.td}>{s.chequeNo || "-"}</td>
                                  <td style={styles.td}>{formatDate(s.chequeDate) || "-"}</td>
                                  <td style={{ ...styles.td, textAlign: "right" }}>
                                    {s.closingBalance != null
                                      ? Number(s.closingBalance).toLocaleString()
                                      : "-"}
                                  </td>
                                </tr>
                              );
                          ...
                          <tfoot>
                            {(() => {
                              const stats = getSalesStats(viewingEntry.salesDetails, viewingEntry.productType);
                              return (
                                <tr style={{ background: "#f1f5ff", fontWeight: "800" }}>
